...
- LEN / SPCX / TSLA are underwater vs cost — **average-down is allowed** only via **[Consider]** when `rules.md` **Average-down gate** passes (thesis intact + **fwd P/S Low with fwd P/E not High** + risk bands + event awareness). Down vs cost alone is not a reason to add. Low P/S is evidence, not a buy.
